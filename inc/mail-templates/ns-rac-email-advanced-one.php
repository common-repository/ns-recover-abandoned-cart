<?php
    function ns_rac_mail_template_advanced_one($ns_title, $ns_cart, $email, $template_color, $button_txt){
        ...
        <table style="padding-top: 20px; margin-top: 50px;">';
        $total_price = 0;
        $i = 0;
        foreach(unserialize($ns_cart[0]->cart) as $prod_id=>$inner_arr){
            ...
            if($i == 1){
                $ns_text .='<tr style="border-bottom: 0px; height: 200px;">';
            }
            $prod = get_post($prod_id);
            $content = $prod->post_content;
            $ns_text .=' <td style="padding-left: 15px; color: #898989;">'.$post_image.'<br><div style="height: 42px;"><b style="color:'.$template_color.';">'.get_the_title($prod_id).'</b> '.$inner_arr['price'].get_woocommerce_currency_symbol().' x('.$inner_arr['quantity'].')</div><br><p style="height: 70px;">'.substr($content, 0, 80) . '...</p></td>';
            if($i == 3){
                $ns_text .='</tr>';
                $i = 0;
            }
        ...
        $ns_text .= '</table>
        <br> <br>
        <p style="font-size: 24px; margin-bottom: 0px;"><b style="color:'.$template_color.';">Total: </b>'.$total_price.get_woocommerce_currency_symbol().'</p>
        <br>
                    ...
                    <td align="center" valign="top">
                        <table border="0" cellpadding="0" cellspacing="0" width="600" id="template_container" style="box-shadow: 0 1px 4px rgba(0,0,0,0.1) !important; background-color: #fdfdfd; border: 1px solid #dcdcdc; border-radius: 3px !important;">
                            <tr>
                                <td align="center" valign="top">
                                    <!-- Header -->
                                    <table border="0" cellpadding="0" cellspacing="0" width="600" id="template_header" style=\'background-color: '.$template_color.'; border-radius: 0px 0px 0 0 !important; width:60%; color: #ffffff; border-bottom: 0; font-weight: bold; line-height: 100%; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif;\'>
                                        <tr>
                                            <td id="header_wrapper" style="padding: 36px 48px; display: block;">
                                                <h1 style=\'color: #ffffff; font-family: "Varela Round", sans-serif; text-align: center; font-size: 30px; font-weight: 300; line-height: 150%; margin: 0; -webkit-font-smoothing: antialiased;\'>'.$ns_title.'</h1>
                                            </td>
                                        </tr>
                                    </table>
                                    <!-- End Header -->
